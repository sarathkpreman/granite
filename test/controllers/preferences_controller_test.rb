...
class PreferencesControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_update_failure_for_invalid_notification_delivery_hour
    ...
    expected_error_message = "Notification delivery hour #{I18n.t('preference.notification_delivery_hour.range')}"

    assert_equal expected_error_message, response.parsed_body["error"]
  end
